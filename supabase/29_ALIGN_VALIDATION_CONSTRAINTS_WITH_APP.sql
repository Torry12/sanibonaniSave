...
-- Cleanup: Nullify or remove rows with invalid fields?
-- For existing transactional data, it's safer to nullify non-critical fields or update them to a safe default if required.
-- However, payouts.account_no and branch_code are NOT NULL in schema.
-- So we update them to a dummy safe value if they are invalid.
...
-- Cleanup: Same strategy for claims
