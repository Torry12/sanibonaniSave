-- seed_20_groups_500_members.sql
-- REAL WORLD SIMULATION SEED SCRIPT v2.0
-- Seeds 20 groups and ~580 members with fully realistic SA savings group patterns.
...
-- ─── 1. AUTH CLEANUP (avoid duplicate auth/profile keys on reruns) ───────────
DO $$
DECLARE
...
BEGIN
    FOR i IN 1..20 LOOP
        ...
        v_type := CASE
            WHEN i <= 8  THEN 'burial_society'
            WHEN i <= 14 THEN 'stokvel'
            WHEN i <= 17 THEN 'investment_club'
            ELSE              'rosca'
        END;
        ...
        v_name := CASE v_type
            ...
            WHEN 'stokvel'         THEN v_stokvel_names[((i - 9) % 6) + 1]
            WHEN 'investment_club' THEN v_invest_names[((i - 15) % 3) + 1]
            ELSE                        v_rosca_names[((i - 18) % 3) + 1]
        END;

        v_bank   := (ARRAY['Capitec','FNB','Standard Bank','Absa','Nedbank'])[(i % 5) + 1];
        v_acc    := LPAD(((i * 123456789) % 9999999999)::BIGINT::text, 10, '0');
...
BEGIN
    ...
    FOR k IN 1..550 LOOP
        ...
        INSERT INTO public.members (
            ...
            CASE WHEN v_g_idx <= 8 THEN 'https://storage.example.com/members/mem' || k || '/benef.pdf' ELSE NULL END,
            CASE WHEN v_g_idx <= 8 THEN 'Beneficiary Form' ELSE NULL END,
            CASE WHEN v_g_idx <= 8 THEN (ARRAY['pending','verified','verified'])[(k % 3) + 1] ELSE NULL END
        ) ON CONFLICT DO NOTHING;
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        ...
        v_hash  := abs(v_m.id::text::hashtext);
    ...
    LOOP
        ...
        FOR j IN 1..v_count LOOP
            INSERT INTO public.beneficiaries (
                ...
                v_m.group_id, v_m.id,
                v_b_firsts[((v_hash + j) % 8) + 1] || ' ' || v_b_lasts[((v_hash * j) % 8) + 1],
                v_rels[((v_hash + j) % 8) + 1],
                v_dobs[((v_hash * j) % 8) + 1],
                v_dobs[((v_hash * j) % 8) + 1] < '1961-01-01'::DATE,
...
BEGIN
    FOR v_m IN
        ...
        WHERE m.status = 'active'
          ...
          AND m.total_paid > 0
          AND abs(m.id::text::hashtext) % 5 = 0
    LOOP
        v_hash      := abs(v_m.id::text::hashtext);
...
BEGIN
    FOR v_g IN SELECT id, balance, bank_name, account_number, branch_code FROM public.groups LOOP
        v_hash := abs(v_g.id::text::hashtext);
        ...
        FOR j IN 1..((v_hash % 3) + 2) LOOP
            INSERT INTO public.payouts (
                ...
                (ARRAY['completed','completed','completed','failed','cancelled'])[((v_hash * j) % 5) + 1],
...
BEGIN
    ...
    FOR v_g IN SELECT id, monthly_contribution FROM public.groups LOOP
        ...
        FOR j IN 1..3 LOOP
            ...
            VALUES (
                v_g.id,
                replace(v_msgs[((v_hash * j) % 10) + 1], '%s', v_g.monthly_contribution::text),
                (ARRAY['email','whatsapp','both'])[(v_hash % 3) + 1],
                v_events[((v_hash * j) % 10) + 1]
            );
        END LOOP;
    END LOOP;
...
BEGIN
    FOR v_g IN SELECT id, balance, monthly_contribution, current_members, type FROM public.groups LOOP
        ...
        v_loss_ratio  := CASE WHEN v_g.type = 'burial_society' THEN 62 + (abs(v_g.id::text::hashtext) % 16)
                              WHEN v_g.type = 'investment_club' THEN 20 + (abs(v_g.id::text::hashtext) % 15)
                              ELSE 35 + (abs(v_g.id::text::hashtext) % 20) END;
        v_reserve     := CASE WHEN v_monthly_inc > 0 THEN LEAST(v_g.balance / v_monthly_inc * 100, 300) ELSE 0 END;
        v_solvency    := LEAST(150 - v_loss_ratio + (abs(v_g.id::text::hashtext) % 20), 200);
...
BEGIN
    ...
    FOR v_m IN SELECT id, group_id FROM public.members WHERE status = 'suspended' LIMIT 30 LOOP
        ...
        VALUES (
            CASE WHEN abs(v_m.id::text::hashtext) % 2 = 0
                 THEN 'b2b2b2b2-b2b2-b2b2-b2b2-b2b2b2b2b2b2'
                 ELSE 'c3c3c3c3-c3c3-c3c3-c3c3-c3c3c3c3c3c3' END,
