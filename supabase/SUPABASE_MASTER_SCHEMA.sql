...
-- Version: 6.8 (Unified & Feature-Complete - June 2026)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Ensure phone column exists for existing tables (Migration safety)
...
-- 5. POLICIES (Group level rules/insurance terms)
...
-- Force phone column to be nullable for existing tables
...
-- 9. PAYMENTS (YoCo/Stitch transactions)
...
-- 12. BENEFICIARY PAYOUT CLAIMS (Burial Society)
...
-- 13. PAYOUTS (ROSCA / Generic Group Disbursals)
...
CREATE TABLE IF NOT EXISTS public.group_actuarial_metrics (
    ...
    expected_annual_claims         NUMERIC(12,2) DEFAULT 0,
    expected_annual_claims_count   NUMERIC(12,2) DEFAULT 0,
    actuarial_present_value        NUMERIC(12,2) DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS public.member_behavior_track (
    ...
    payment_consistency_score   NUMERIC(5,2) DEFAULT 0,
    average_days_late           NUMERIC(5,2) DEFAULT 0,

    -- Streak Metrics
    current_payment_streak      INTEGER DEFAULT 0,
    longest_payment_streak      INTEGER DEFAULT 0,
    has_broken_streak_recently  BOOLEAN DEFAULT FALSE,

    -- Financial Metrics
    total_amount_contributed    NUMERIC(15,2) DEFAULT 0,
    total_late_fees_paid        NUMERIC(12,2) DEFAULT 0,
    pending_late_fees           NUMERIC(12,2) DEFAULT 0,
    total_outstanding_amount    NUMERIC(15,2) DEFAULT 0,

    -- Loan Metrics
    total_loans_requested       INTEGER DEFAULT 0,
    total_loans_approved        INTEGER DEFAULT 0,
    total_loans_completed       INTEGER DEFAULT 0,
    active_loans                INTEGER DEFAULT 0,
    overdue_loans               INTEGER DEFAULT 0,
    loan_default_count          INTEGER DEFAULT 0,
    loan_completion_rate        NUMERIC(5,2) DEFAULT 0,

    -- Fraud Indicators
    duplicate_transaction_count INTEGER DEFAULT 0,
    suspicious_activity_count   INTEGER DEFAULT 0,
    unusual_payment_patterns    BOOLEAN DEFAULT FALSE,
    multiple_accounts_detected  BOOLEAN DEFAULT FALSE,
    velocity_check_failed       BOOLEAN DEFAULT FALSE,
    rapid_disbursement_attempts INTEGER DEFAULT 0,

    -- Behavioral Flags
    member_status               TEXT NOT NULL DEFAULT 'fair', -- BehaviorStatus: excellent, good, fair, poor, suspended
    fraud_risk_level            TEXT NOT NULL DEFAULT 'LOW',  -- FraudRiskLevel: low, medium, high, critical
    ...
    -- Duration
    months_in_group             INTEGER NOT NULL DEFAULT 0,
...
-- 17. MEMBER DOCUMENTS (Relational Document Store)
...
-- 19. VIRTUAL BANK (Simulated external banking)
CREATE TABLE IF NOT EXISTS public.virtual_bank_accounts (
    id                   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.virtual_bank_transactions (
    id                   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_account_id    UUID REFERENCES public.virtual_bank_accounts(id),
    destination_account_id UUID REFERENCES public.virtual_bank_accounts(id),
...
CREATE INDEX IF NOT EXISTS idx_group_health_scores_group_id ON public.group_health_scores(group_id);
