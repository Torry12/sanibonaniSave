...
class ValidateLoanEligibilityUseCase @Inject constructor(
    ...
    suspend operator fun invoke(member: Member, group: Group): EligibilityResult {
        ...
        if (monthsJoined < 6) {
            return EligibilityResult.Ineligible("You must be a member for at least 6 months to qualify for a loan (Current: $monthsJoined months).")
        }
        ...
        if (calculation.shortfall > 0 || calculation.isOverdue) {
            return EligibilityResult.Ineligible("You must be up to date with your monthly contributions to qualify for a loan. (Shortfall: R${calculation.shortfall})")
        }
